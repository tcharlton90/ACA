...
int decodedEnd = 0, procClock = 1;
char * operation[] =  {"ADD","SUB","MUL","DIV","CMP","MOV","LDR","STR","B","BLT","BE","BGT","JMP","RTN","END","NOP","","","","","","","","","Bi","BLTi","BGTi","BEi"};
...
{
	int a, b, c, i, dependanciesFound, first = 0; // a, b and c are for registers
	struct issueBuffer * tmp;
	...
	while (tmp != NULL)
	{
		if (strcmp(tmp -> instructionForIssue -> opcode, "01111") != 0)
		{
			...
			if (strncmp(tmp -> instructionForIssue -> opcode, "110", 3) == 0)
			{
				//stall  a branch
				if (scoreBFlags > 0)
				{
			...
			if (dependanciesFound == 0 || !first)
			{
				tmp -> dependancies = 0;
			}
			
			//add dependancies to
			if (a >= 0 && scoreBoard[a] == 0 && first)
			{
				scoreBoard[a] = penalty;
			}
			if (b >= 0 && scoreBoard[b] == 0 && first)
			{
				scoreBoard[b] = penalty;
			}
			if (c >= 0 && scoreBoard[c] == 0 && first)
			{
				scoreBoard[c] = penalty;
			}
			first = 1;
		}
		tmp = tmp -> next;
